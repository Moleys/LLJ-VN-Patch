...
UI: descramble (bin\KirikiriDescrambler.exe) → dịch → `apply_uitexts2.py` → `scramble_mode1.py` → đưa vào staging `unencrypted\`.

# 5) đóng gói phân phối — gom staging thành 1 file xp3
bin\Xp3Pack.exe <staging>\unencrypted     # -> <staging>\unencrypted.xp3
# copy unencrypted.xp3 + version.dll vào game root là cài đặt.
